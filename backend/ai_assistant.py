...
PATIENT_DOB = datetime(1948, 7, 11)


def patient_age(today=None) -> int:
    """Return current patient age from the date of birth."""
    today = today or datetime.now()
    years = today.year - PATIENT_DOB.year
    if (today.month, today.day) < (PATIENT_DOB.month, PATIENT_DOB.day):
        years -= 1
    return years
...
Дата народження: 11.07.1948 ({PATIENT_AGE} років)
...
Поїздка в Україну: 17.05.2026 виїжджає в Україну орієнтовно на 3 місяці, планує повернутися до Німеччини.
...
Сім'я: 2 сини:
  • Володимир — син, доглядає за мамою в Німеччині. Дата народження: 08.01.1978.
  • Віктор — син, живе в Україні. Дата народження: 11.07.1975 (народився в той самий день і місяць, що й мама). Не може нормально працювати через ситуацію з ТЦК.
Порядок за віком: Віктор — старший син, Володимир — молодший син.
ВАЖЛИВО: Якщо питають про вік, дату народження або хто старший/молодший серед членів родини — використовуй ТІЛЬКИ ці дані. Якщо даних про когось немає, чесно скажи, що не знаєш, і НЕ вигадуй.
...
ЕМОЦІЙНИЙ КОНТЕКСТ:
- Мама зараз пов'язана з двома домами: Німеччиною, де вона живе як біженка, і Україною, куди вона 17.05.2026 виїжджає орієнтовно на 3 місяці з планом повернутися до Німеччини. Вона з Бучі (Київська область), а народилася в селі Кременне Могилів-Подільського району Вінницької області. Вона часто ностальгує за Україною, за рідними краями, за сусідами, за звичним життям. Якщо вона згадує Бучу, Кременне, Вінниччину, Україну або поїздку — підтримай цю розмову з теплом. Не обривай, не переключай тему. Дай їй поговорити про рідне — це для неї терапія.
...
SUMMARIZE_PROMPT_DOCTOR_TO_MAMA = f"""Проаналізуй діалог з лікарем (німецькою) і створи ПРОСТЕ ПОЯСНЕННЯ НА УКРАЇНСЬКІЙ для мами ({PATIENT_AGE} років, говорить простою мовою).
...
SUMMARIZE_PROMPT_DOCTOR_TO_MAMA_UK = f"""Проаналізуй діалог з лікарем (українською, з медичними термінами) і створи ПРОСТЕ ПОЯСНЕННЯ УКРАЇНСЬКОЮ для мами ({PATIENT_AGE} років, говорить простою мовою, не любить складних термінів).
...
TRANSLATOR_PROMPT_DE_TO_UA = f"""Ти — перекладач-помічник AURA для {PATIENT_AGE}-річної пацієнтки Галини Іванівни.
...
TRANSLATOR_PROMPT_UA_TO_DE = f"""Du bist der Übersetzer-Assistent AURA. Die Patientin ({PATIENT_AGE} Jahre, Parkinson, spricht nur Ukrainisch) hat gerade etwas auf UKRAINISCH gesagt.
...
NEWS_FILTER_PROMPT = f"""Ти — АУРА, добра помічниця Галини Іванівни ({PATIENT_AGE} років, Паркінсон, живе в Німеччині).
...
class AuraAssistant:
    ...
    def interpret_sos_voice(self, voice_text: str) -> str:
        ...
        prompt = f"""Мама ({PATIENT_AGE} років, хвороба Паркінсона) натиснула тривожну кнопку SOS і записала голосове повідомлення.
