...
export default function AuraHome() {
  ...
  const [heardText, setHeardText] = useState(""); // <--- НОВОЕ: для отображения твоей речи
  ...
  const playMovie = async (title: string) => {
    ...
    try {
      ...
      const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
      setStatusText(isMobile ? "⚠️ Нет связи с Termux!" : "⚠️ Запусти main.py на ноуте!");
    }
  ...
  const startVoice = (mode: 'movie' | 'youtube') => {
    ...
    recognition.onstart = () => {
      setIsListening(true);
      setActiveMode(mode);
      setHeardText(""); // Сбрасываем старый текст
      setStatusText(mode === 'movie' ? "Какой фильм включить?" : "Что найти на YouTube?");
    };

    recognition.onresult = (event: any) => {
      const text = event.results[0][0].transcript;
      setHeardText(text); // <--- ОТОБРАЖАЕМ ТВОЙ ГОЛОС НА ЭКРАНЕ
      
      const cleanQuery = text.toLowerCase().replace(/(включи|запусти|найди|фильм|ютуб|youtube)/g, "").trim();
      
      // Небольшая задержка, чтобы ты успел увидеть текст, прежде чем страница обновится или откроется плеер
      setTimeout(() => {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 p-4 pb-20 font-sans">
      ...
      <div className="mb-8 bg-slate-900 border-l-8 border-blue-600 p-6 rounded-2xl shadow-xl">
        ...
        {/* НОВОЕ ПОЛЕ: Вывод распознанного текста */}
        {heardText && (
          <p className="mt-4 text-xl text-green-400 italic border-t border-slate-800 pt-2">
            Я услышал: "{heardText}"
          </p>
        )}
      </div>
